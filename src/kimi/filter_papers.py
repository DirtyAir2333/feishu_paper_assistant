...
def filter_by_gpt(
    ...
    with open(CONFIGS_DIR / "paper_topics.md", "r") as f:
        criterion = f.read()
    with open(CONFIGS_DIR / "postfix_prompt.md", "r") as f:
    ...
    if config["SELECTION"].getboolean("run_openai"):
        ...
        if config["OUTPUT"].getboolean("dump_debug_file"):
            with open(
                OUTPUT_DIR / "gpt_paper_batches.debug.json", "w"
            ) as outfile:
...
if __name__ == "__main__":
    ...
    with open(PROJECT_ROOT / "in" / "debug_papers.json", "r") as f:
    ...
    with open(
        OUTPUT_DIR / "filter_paper_test.debug.json", "w"
    ) as outfile:
