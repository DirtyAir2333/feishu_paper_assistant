...
# On GitHub Actions, these will be set via repository secrets
load_dotenv()
...
from .push_to_feishu import push_to_feishu
...
def get_authors(
    ...
    with Session() as session:
        for author in tqdm(all_authors):
            ...
            # with S2_KEY: 500ms (2 requests/sec)
            # without S2_KEY: 1s (1 request/sec)
...
def parse_authors(lines):
    # parse the author list from .md format: "Name, ID # comment"
    # ignoring lines that are empty, starting with #, or markdown headers
    author_ids = []
    authors = []
    for line in lines:
        line = line.strip()
...
if __name__ == "__main__":
    ...
    # Only query first author and last author (assumed corresponding author) to reduce API calls
    all_authors = set()
    for paper in papers:
        if paper.authors:
            all_authors.add(paper.authors[0])  # first author
            if len(paper.authors) > 1:
                all_authors.add(paper.authors[-1])  # last author (corresponding)
    if config["OUTPUT"].getboolean("debug_messages"):
        print("Getting author info for " + str(len(all_authors)) + " authors (first + last only)")
    all_authors = get_authors(list(all_authors), S2_API_KEY)

    if config["OUTPUT"].getboolean("dump_debug_file"):
        with open(
            OUTPUT_DIR / "papers.debug.json", "w"
        ) as outfile:
            json.dump(papers, outfile, cls=EnhancedJSONEncoder, indent=4)
        with open(
            OUTPUT_DIR / "all_authors.debug.json", "w"
        ) as outfile:
            json.dump(all_authors, outfile, cls=EnhancedJSONEncoder, indent=4)
        with open(
            OUTPUT_DIR / "author_id_set.debug.json", "w"
        ) as outfile:
    ...
    if len(papers) > 0:
        ...
        if config["OUTPUT"].getboolean("push_to_slack"):
            if os.environ.get("FEISHU_APP_ID") is None or os.environ.get("FEISHU_APP_SECRET") is None:
                print(
                    "Warning: push_to_slack is true, but FEISHU_APP_ID or FEISHU_APP_SECRET is not set - not pushing to Feishu"
                )
            else:
                push_to_feishu(selected_papers)
